        ...
        if (flag) {
            $.ajax({
                ...
                success: function (response) {
                    if (response === "success") {
                        $("#replace").html('');
                        $("#AddResult").html('<h6 class="text-success">Thêm Blog mới thành công</h6>');
                    } else {
                        console.log(response);
                        $("#AddResult").html('<h6 class="text-danger">Ảnh vượt quá 10MB</h6>');
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    $("#replace").html('<button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#AddModel">Quay lại</button>');
                    $("#AddResult").html('<h6 class="text-danger">Thêm địa điểm mới không thành công</h6>');
                }
            });
        }
    });
